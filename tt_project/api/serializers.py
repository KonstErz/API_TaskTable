...
class TaskCreationSerializer(serializers.Serializer):
    ...
    def validate_name(self, value):
        if Task.objects.filter(name=value).exists():
            raise serializers.ValidationError(
                'a task with the same name already exists, try a another name')
    ...
    def validate(self, values):
        if len(values['name']) > 200:
            raise serializers.ValidationError(
                'task name must be no more than 200 characters')

        elif values['specification'] is None:
            raise serializers.ValidationError(
                'task specification should be indicated')
...
class TaskUpdateSerializer(serializers.Serializer):
    """
    Serializes the data to update an existing task.
    Conducts object- and field-level validation.
    """
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    Task creator only HTML form.
    """

    class Meta:
        model = Task
        fields = ['url', 'id', 'name', 'specification', 'due_date', 'creator',
                  'performer', 'status']
        read_only_fields = ['creator']


class TaskPerformerSerializer(serializers.ModelSerializer):
    """
    Serializes data to display a Task model.
    Task performer only HTML form.
    """
    ...
    class Meta:
        ...
        fields = ['url', 'id', 'name', 'specification', 'due_date', 'creator',
                  'performer', 'status']
        read_only_fields = ['creator', 'performer']
